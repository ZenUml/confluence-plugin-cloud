...
describe('Macro', () => {
  ...
  describe('load content when initialising', () => {
    ...
    it('should default to example', async () => {
      ...
      expect(gtag.mock.calls).toEqual([
        ['event', 'new_macro', {event_category: 'custom_content', event_label: 'sequence',
          client_domain: 'unknown_atlassian_domain', user_account_id: 'unknown_user_account_id', confluence_space: 'unknown_space'}]
      ])
    })
    ...
    test('or content property', async () => {
      ...
      expect(gtag.mock.calls).toEqual([
        ['event', 'load_macro', {
          event_category: 'content_property', event_label: '_sequence',
          client_domain: 'unknown_atlassian_domain', user_account_id: 'unknown_user_account_id', confluence_space: 'unknown_space'}],
      ])
    })
    ...
    test('should fallback to macro body when content property fails', async () => {
      ...
      expect(gtag.mock.calls).toEqual(expect.arrayContaining([
        ['event', 'get_content_property', {event_category: 'warning', event_label: 'property is not found with key:zenuml-sequence-macro-1234-body', client_domain: 'unknown_atlassian_domain', confluence_space: 'unknown_space', user_account_id: 'unknown_user_account_id'}],
      ]));
    })
    ...
    test('should throw error if fallback to macro body also fails', async () => {
      ...
      expect(gtag.mock.calls).toEqual(expect.arrayContaining([
        ['event', 'get_content_fallback', {event_category: 'error', event_label: 'Fallback to macro body failed', client_domain: 'unknown_atlassian_domain', confluence_space: 'unknown_space', user_account_id: 'unknown_user_account_id'}],
      ]));
    })
