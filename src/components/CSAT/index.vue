<template>
  <transition name="fade">
    <div
      v-if="open"
      class="fixed w-[470px] h-fit bottom-4 right-4 z-50 bg-white text-[#172B4D] p-10 rounded-lg drop-shadow-[0_20px_26px_rgba(176,176,176,0.35)]"
    >
      <div v-if="!hasFeedback">
        ...
        <div class="mt-3 flex justify-between text-sm">
          <div
            ...
            class="bg-[#F1F1F1] w-12 h-12 rounded-lg border-2 p-1.5 text-2xl flex items-center justify-center group hover:bg-[#e0e0e0] duration-200 cursor-pointer"
            :class="csatVal === index + 1 ? 'border-[#444BFF]' : 'border-transparent'"
          >
            {{ value }}
          </div>
        </div>
        <div class="mt-2.5 text-xs text-[#939393] flex justify-between">
          <span>Very bad</span>
          <span>Very good</span>
        </div>
        <div class="flex text-xs mt-10 justify-center">
          ...
          <div v-else class="flex w-full text-sm gap-8 items-center cursor-pointer">
            ...
            <a
              ...
              class="px-7 flex justify-center h-[52px] items-center flex-1 py-3 bg-[#282828] rounded-lg text-white"
      ...
      <div>
        <transition name="fade-no-transform">
          <div
            class="bg-inherit rounded-lg flex flex-col"
          ...
          >
            <div class="flex gap-2">
              ...
              <span class="text-center text-3xl">Thank You</span>
            </div>
